...
const StoresList = ({ stores, onSelect }) => {
  ...
  const filtered = useMemo(() => stores.filter(s => { if (city && s.city !== city) return false; if (net && s.network !== net) return false; if (driver && s.driver !== driver) return false; if (agent && s.agent !== agent) return false; if (status && s.status !== status) return false; return true; }), [stores, city, net, driver, agent, status]);
  const cols = [
...
const StoreDetail = ({ store, onBack }) => {
  ...
  return (<div className="space-y-6">
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 border"><div className="flex justify-between items-start"><div><h1 className="text-2xl font-bold">{store.name}</h1><p className="text-gray-500 mt-1">{store.city} {store.network && `• ${store.network}`}</p><p className="text-sm text-gray-400 mt-1">נהג: {store.driver || '-'} | סוכן: {store.agent || '-'}</p></div><div className="flex gap-2"><Badge status={store.status} /><Badge status={store.status} /></div></div></div>
...
const ProductsList = ({ products, onSelect }) => {
  ...
  const [status, setStatus] = useState('');
  const filtered = useMemo(() => products.filter(p => { if (cat && p.category !== cat) return false; if (status && p.status !== status) return false; return true; }), [products, cat, status]);
  const cols = [
    { k: 'name', l: 'מוצר', r: (v, r) => <div><p className="font-medium">{v}</p><p className="text-xs text-gray-500">{r.category}</p></div> },
    { k: 'metric_12v12', l: '12/12', r: (v, r) => <MetricCell pct={v} from={r.qty_2024} to={r.qty_2025} /> },
    { k: 'metric_6v6', l: '6/6', r: (v, r) => <MetricCell pct={v} from={r.qty_prev6} to={r.qty_last6} /> },
...
const ProductDetail = ({ product, onBack }) => {
  ...
  return (<div className="space-y-6">
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 border"><div className="flex justify-between items-start"><div><h1 className="text-2xl font-bold">{product.name}</h1><p className="text-gray-500 mt-1">{product.category}</p></div><div className="flex gap-2"><Badge status={product.status} /><Badge status={product.status} /></div></div></div>
